...
import (
	...
	"github.com/uzlocoin/multiwallet/datastore"
...
func NewDefaultConfig(coinTypes map[wallet.CoinType]bool, params *chaincfg.Params) *Config {
	...
	if coinTypes[util.CoinTypeUzlocoin.ToCoinType()] {
		var apiEndpoints []string
		if !testnet {
			apiEndpoints = []string{
				"https://phr.blockbook.api.uzlocoin.io/api",
			}
		} else {
			apiEndpoints = []string{
				"https://tphr.blockbook.api.uzlocoin.io/api",
			}
		}
		db, _ := mockDB.GetDatastoreForWallet(util.CoinTypeUzlocoin.ToCoinType())
		phrCfg := CoinConfig{
			CoinType:    util.CoinTypeUzlocoin,
			FeeAPI:      "",
			SuperLowFee: 70,
			LowFee:      140,
			MediumFee:   160,
